...
export function BuilderSidebar() {
  ...
  // Open sidebar automatically on mobile when a block is selected
  useEffect(() => {
    if (selectedBlockId) {
      setIsOpen(true);
    }
  ...
  return (
    <>
      ...
      {/* Sidebar Panel */}
      <aside className={`
        fixed inset-y-0 right-0 z-40 w-80 bg-zinc-950 border-l border-white/5 flex flex-col h-full overflow-hidden transition-transform duration-300
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 pb-24 md:pb-4">
              {BLOCK_TYPES.map((type) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 p-3 rounded-xl bg-white/[0.03] hover:bg-white/[0.08] border border-white/5 hover:border-white/10 transition-all group text-left cursor-pointer"
                >
